...
// NewQuietServer is an easy helper method to create a new Consul
// test server that doesn't flood the log
func NewQuietServer() (*TestServer, error) {
	return newTestServerConfigT(nil, nil, true, true)
}
...
func newTestServerConfigT(t *testing.T, cb ServerConfigCallback, shutup ...bool) (*TestServer, error) {
	...
	// Start the server
	go consulrun.Cmd(&consulrun.Config{Stdout: stdout, Stderr: stderr}, args...)
	time.Sleep(time.Second)

	// TODO(as): This consul API seems to have no support for cancellation
	// which means that the agent _cant be stopped_
